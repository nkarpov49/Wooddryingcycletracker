...
export default function CycleDetail() {
  ...
  async function loadCycle(cycleId: string) {
    try {
      ...
      console.error(err);
      setError(t('error') || 'Ошибка загрузки');
  ...
  const handleCopy = () => {
    if (!cycle) return;
    navigate("/admin/new", { state: { copyFrom: cycle } });
  };

  if (loading) {
    return <div className="p-8 text-center text-xl">{t('loading') || 'Загрузка...'}</div>;
  }
              ...
              {cycle.startDate && cycle.endDate && (
                <div className="mt-4 pt-4 border-t border-gray-100 text-center">
                  ...
                  <p className="font-bold text-gray-900">
                    {differenceInHours(new Date(cycle.endDate), new Date(cycle.startDate))} {t('hoursShort')}
                  </p>
                </div>
              )}
            </div>
