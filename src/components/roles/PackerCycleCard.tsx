...
export default function PackerCycleCard({ cycle, onClick }: PackerCycleCardProps) {
  ...
  const getWoodStyle = (woodType: string) => {
    ...
    if (type.includes('beržas')) return 'bg-blue-100 text-blue-800 border-blue-200';
    if (type.includes('ąžuolas')) return 'bg-yellow-100 text-yellow-800 border-yellow-200';
    if (type.includes('alksnis')) return 'bg-gray-100 text-gray-800 border-gray-200';
    if (type.includes('uosis') || type.includes('ash')) return 'bg-green-100 text-green-800 border-green-200';
    if (type.includes('klevas') || type.includes('ash')) return 'bg-green-100 text-green-800 border-green-200';
